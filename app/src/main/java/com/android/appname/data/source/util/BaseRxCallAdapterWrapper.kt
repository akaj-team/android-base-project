...
open class BaseRxCallAdapterWrapper<R>(
    ...
    override fun adapt(call: Call<R>): Any? {
        return when (val adapt = wrapped?.adapt(call)) {
            is Maybe<*> -> adapt.map(this::handleCustomError)
                .onErrorResumeNext(this::handleMaybeRetrofitError)
            is Single<*> -> adapt.map(this::handleCustomError)
                .onErrorResumeNext(this::handleSingleRetrofitError)
            is Flowable<*> -> adapt.map(this::handleCustomError)
                .onErrorResumeNext(this::handleFlowableRetrofitError)
            is Observable<*> -> adapt.map(this::handleCustomError)
                .onErrorResumeNext(this::handleObservableRetrofitError)
            else -> CustomCallAdapter(call, retrofit)
        }
    }
    ...
    open fun convertRetrofitExceptionToCustomException(
        throwable: Throwable,
        retrofit: Retrofit
    ): Throwable? = throwable
